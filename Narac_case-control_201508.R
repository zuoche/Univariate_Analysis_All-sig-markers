...
write.table(marker, "Narac_all-marker_case-ctrl.txt", quote=F, col.names=F, row.names=F)

#univariate logistic regression for summerized 39 markers
...
write.table(narac, "Narac_all-marker_case-ctrl.txt", quote=F, row.names=F, col.names=F, sep="\t")

narac_bi <- ifelse(narac[,2:17]<0.05/39, 1, 0)
narac_bi <- data.frame(narac[,1], narac_bi[,1:16])
write.table(narac_bi, "Narac_all-marker_bi_case-ctrl.txt", quote=F, row.names=F, col.names=F, sep="\t")
...
names(tmp) <- ab
pheatmap(test, scale="column", clustering_distance_col="correlation", cluster_rows=T, cluster_cols=T, fontsize=15, fontsize_rows=5)
